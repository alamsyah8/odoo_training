...
class PlanningSlotTraining(models.Model):
    ...
    resource_ids = fields.Many2many(
        "resource.resource", string="Resource", compute="_compute_resource_ids"
    )
    ...
    start_date = fields.Date(
        "Start Date", default=lambda self: fields.Date.context_today(self)
    )
    ...
    number_of_mandays = fields.Integer(
        compute="_compute_number_of_mandays", string="Number of Mandays", store=True
    )
    point_rate = fields.Integer(
        compute="_compute_role_components", string="Point Rate", store=True
    )
    amount = fields.Monetary(
        "Amount",
        compute="_compute_role_components",
        store=True,
        curreny_field="currency_id",
    )
    total_points = fields.Integer(
        compute="_compute_total_points", string="Total Points", store=True
    )
    ...
    line_ids = fields.One2many(
        "planning.slot.training.line", "slot_training_id", string="Line"
    )
    ...
    def _compute_actual_progress(self):
        for rec in self:
            if rec.line_ids:
                line_ids = rec.line_ids.filtered(
                    lambda x: x.state == "confirm" and x.current_progress > 0
                )
                actual_progress = (
                    line_ids.sorted(key=lambda x: (x.date, x.id))[-1].current_progress
                    if line_ids
                    else 0
                )
...
class PlanningSlotTrainingLine(models.Model):
    ...
    project_update_id = fields.Many2one(
        "project.update", string="Project Update", index=True, copy=False
    )
    project_id = fields.Many2one(
        "project.project",
        string="Project",
        index=True,
        store=True,
        related="project_update_id.project_id",
    )
    slot_training_id = fields.Many2one(
        "planning.slot.training",
        string="Slot Training",
        index=True,
        copy=False,
        tracking=True,
    )
    ...
    role_id = fields.Many2one(
        "planning.role.training", string="Role", related="slot_training_id.role_id"
    )
    resource_id = fields.Many2one(
        "resource.resource", string="Resource", related="slot_training_id.resource_id"
    )
