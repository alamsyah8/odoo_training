...
class ProjectUpdate(models.Model):
    ...
    def _get_default_line_ids(self):
        ...
        if (
            ...
            and not "project.project" in self._context.get("active_model")
        ):
            return res
        project_id = (
            [self.project_id.id] if self.project_id else self._context.get("active_ids")
        )
        result = self.env["project.project"].browse(project_id)
        if not result:
            return res
        for record in result:
            if not record.planning_shift_line_ids:
                return res
    ...
    def _prepare_values(self, record, date=False):
        ...
        for rec in record.planning_shift_line_ids:
            ...
            update_line_ids = (
                line_ids.sorted(key=lambda r: (r.date, r.id))[-1] if line_ids else []
            )
            date_condition = (
                False
                if (not date) or (not update_line_ids)
                else bool(update_line_ids.date > date)
            )
            previous_progress = (
                0.0
                if update_line_ids and date_condition
                else update_line_ids.current_progress if update_line_ids else 0.0
            )
            res.append(
                (
                    0,
