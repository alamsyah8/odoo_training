...
class ProjectProject(models.Model):
    ...
    planning_shift_line_ids = fields.One2many(
        "planning.slot.training", "project_id", string="Planning Shift Line"
    )
    planning_shift_count = fields.Integer(
        compute="_compute_planning_shift_count", string="Planning Shift Count"
    )
    ...
    project_actual_progress = fields.Float(
        compute="_compute_project_actual_progress",
        string="Actual Progress",
        digits=(6, 2),
    )
    project_actual_revenue = fields.Monetary(
        compute="_compute_project_actual_revenue",
    ...
    def _compute_project_actual_revenue(self):
        for rec in self:
            rec.project_actual_revenue = sum(
                rec.planning_shift_line_ids.mapped("actual_revenue")
            )

    @api.depends(
        "planning_shift_count",
        "planning_shift_line_ids.actual_progress",
        "planning_shift_line_ids.point_rate",
    )
    def _compute_project_actual_progress(self):
        for rec in self:
            planning_shift_count = (
                rec.planning_shift_count if rec.planning_shift_count else 1
            )
            project_actual_progress = round(
                sum([record.actual_progress for record in rec.planning_shift_line_ids])
                / planning_shift_count,
                2,
            )
    ...
    def _compute_expected_revenue(self):
        for rec in self:
            rec.expected_revenue = sum(
                rec.planning_shift_line_ids.mapped("expected_revenue")
            )
    ...
    def action_view_planning_shift(self):
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "sanbe_farma_training.project_planning_slot_action"
        )
    ...
    def action_project_reporting_wizard(self):
        ...
        return {
            ...
            "view_id": self.env.ref(
                "sanbe_farma_training.project_reporting_wizard_view_form"
            ).id,
            "target": "new",
            "context": context,
        }
