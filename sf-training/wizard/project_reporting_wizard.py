...
class BaseExcelReporting(models.AbstractModel):
    ...
    def _set_values(
        self, worksheet, wbf, content_line, header, column_length, vals, linenum
    ):
        ...
        for col_length in range(column_length):
            ...
            if column_name == "no":
                worksheet.write(
                    content_line, length[col_length], linenum, wbf["content"]
                )
            elif column_name in vals:
                worksheet.write(
                    content_line, length[col_length], vals[column_name], wbf["content"]
                )
...
class ProjectReportingWizard(models.TransientModel):
    ...
    def _prepare_excel_template_1(self):
        ...
        for project in self.project_ids:
            ...
            if self.template == "template_2":
                total = sum(
                    [record["expected_revenue"] for record in planning_shift_values]
                )
                # merge_range(0, 1, 0, 8)
                # 0 : Posisi Baris ke - 1
                # 1 : Posisi Kolom ke - A,
                ...
                worksheet.merge_range(
                    content_line + 1, 0, content_line + 2, 6, "Total", wbf["header"]
                )
